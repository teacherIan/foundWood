  ...
  useEffect(() => {
    ...
    if (isInitialMountRef.current) {
      ...
      if (initialLoadComplete) {
        console.log(
          '🎬 LOADING COMPLETE - Canvas now visible, starting 5-second alpha animation from 0.8 → 0.3'
        );

        const startTime = Date.now();
        const animationDuration = 5000; // 5-second animation from 0.8 to 0.3 for optimal rendering

        const animateAlpha = () => {
          const elapsedTime = Date.now() - startTime;
          const progress = Math.min(elapsedTime / animationDuration, 1);
          const currentAlpha = 0.8 + (0.3 - 0.8) * progress; // Interpolate from 0.8 to 0.3
      ...
      if (hasOverlay) {
        ...
        targetAlpha = 0.3; // Animate back to 0.3 when all overlays are hidden for optimal rendering
